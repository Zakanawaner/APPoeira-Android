...
public class MyGroupModificationStudentsMembersRecyclerViewAdapter extends RecyclerView.Adapter<MyGroupModificationStudentsMembersRecyclerViewAdapter.ViewHolder> {

    private Context ctx;
    private List<SearchContent> mValues;
    private OnGroupModificationStundentMembersListener myOnGroupModificationInviteMembersListener;

    public MyGroupModificationStudentsMembersRecyclerViewAdapter(Context context, List<SearchContent> items, OnGroupModificationStundentMembersListener onGroupDetailListener) {
        mValues = items;
        ctx = context;
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (((GroupModificationActivity) ctx).checkMembers(holder.mItem.getId(), false)){
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        OnGroupModificationStundentMembersListener onGroupModificationInviteMembersListener;

        public ViewHolder(View view, OnGroupModificationStundentMembersListener onGroupModificationInviteMembersListener) {
            super(view);
            mView = view;
    ...
    public interface OnGroupModificationStundentMembersListener {
        void OnGroupStudentMemberClick(int position);
    }
    private void toggleSelection(boolean[] invited, Integer id, ImageView ivAdd, ConstraintLayout clLayout) {
        if (!invited[0]) {
            invited[0] = true;
            ((GroupModificationActivity) ctx).addInvited(id, false);
            ivAdd.setImageResource(R.drawable.ic_close);
            clLayout.setBackgroundColor(Color.parseColor("#ABFFA6"));
        } else {
            invited[0] = false;
            ((GroupModificationActivity) ctx).deleteInvited(id, false);
